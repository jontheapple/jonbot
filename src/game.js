...
try {
	fs.accessSync("./savedata.json", fs.constants.R_OK | fs.constants.W_OK);
} catch (err){
	fs.writeFileSync("./savedata.json", "[]");
}

//returns an array "saves" from the file
function readSaves(channel){
	let saves;
	try {
		const jsonString = fs.readFileSync("./savedata.json");
		saves = JSON.parse(jsonString)
	} catch (err){
		reportError("GAME1", channel);
		console.log(err);
	}
	return saves;
}
...
//resets the player to a newgame state, and starts the game
async function newGame(player, channel){
	const saves = readSaves(channel);
	try {
		let playerIndex = getPlayerIndex(saves, player.id);
		if (playerIndex === -1){
			saves.push({"id" : player.id, "location" : 0, "power" : true, "twiddled" : false, "gaming" : true})
			writeSaves(channel, saves);
		} else {
			saves[playerIndex].location = 0;
			saves[playerIndex].power = true;
			saves[playerIndex].twiddled = false;
			saves[playerIndex].gaming = true;
			writeSaves(channel, saves);
		}
	} catch (err){
		reportError("GAME3", channel);
		console.log(err);
	}
...
function isGaming(player, channel){
	const saves = readSaves(channel);
	//if the player is not in the saves, clearly the player is not gaming
	if (getPlayerIndex(saves, player.id) === -1) return false;
...
async function gameLoop(player, content, channel){
	const saves = readSaves(channel);
	playerIndex = getPlayerIndex(saves, player.id);
	const save = saves[playerIndex];
	save.lastEntry = content;

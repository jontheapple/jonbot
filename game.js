...
var game = {
	...
	"gameLoop" : async function doTheThing(content, channel){
		switch(location){
			...
			case 9:
				if (content === "next"){
					channel.send("**Mysterious man**: Jonbot is no ordinary robot. His continued existence is of utmost importance to this world. I am an agent of the United Mech Institute, dedicated to the protection of entities such as Jonbot from those that would seek to harm them.\n" +
						...
						"**You**: ...Fair point. Where are these components located?\n" +
			...
			case 11:
				if (content === "1"){
					channel.send("What's this? Why is it so difficult to walk? Something tells you that you're going about all of this the wrong way. You look closer at the rock and feel like you can see some near-transparent letters. You squint and barely make out that it says \"2 x 2 = ?\". You blink again, and it's gone. That was weird.");
			...
			case 14:
				if (content === "blue" || content === "dark blue"){
					channel.send("You're not quite sure how confident you are in the security here. Literally EVERYONE knows Jonbot's favorite hair color. Oh well, the door opens and you walk in. Turns out, this temple only has one room, and the Jonbot part is right there in the middle of the room. Hopefully there aren't any traps or anything. You walk up to the part and pick it up.");
					await channel.send({file: "./images/robot arm.jpg"});
					channel.send("It's just a normal looking robot arm. Actually, it looks WAY worse than the one you remember seeing on Jonbot's arm. Nonetheless, you grab it anyway. Suddenly, you hear the temple door slam shut. Startled, you turn around, but the world starts shifting and shimmering again. Still not wanting to get motion sick, you close your eyes again.");
					channel.send("Chat \"next\" to continue");
					location = 15;
				} else {
			...
			case 17:
				...
				break;
			case 18:
				if (content === "4017103columbia"){
					await channel.send("HOLY...a TRIPLE question! That one was challenging! Especially that first part. Luckily you knew (or googled or guessed) that 1900 was not a leap year, despite being divisible by 4.", {file: "./images/rabbit.png"});
			...
			case 21:
				if (content === "next"){
					...
					channel.send("**Pirate**: Arr matey! Our sails on the mast are tangled. Can you climb up there and fix it?\n" +
						"**You**: Wait...no, you can't just say \"Arr matey!\" and then talk normally after that. It doesn't work that way.\n" +
						"**Pirate**: Yeah? Well, go talk to the guy who made this dumb game, he was the one who couldn't be bothered to keep up with the pirate-speak. Now go and fix the sails, or I'll make you walk the plank!");
					channel.send("You decide to not fight this guy, especially when you suddenly have a cute little rabbit to take care of. Looking up, you see that one of the sails is indeed tangled with some of the wires. There's a rope ladder that goes up the mast, and most of the steps are made of...well...rope. But one step in particular near the bottom is made of...words? Upon closer inspection, you notice it reads \"But, soft! What light through ?????? window breaks?\"");
					location = 22;
				} else {
					channel.send("Chat \"next\" to continue");
				}
			...
			case 22:
				if (content === "yonder"){
					await channel.send("You reach the top of the mast. Whoa, the rocking of the ship is way more violent up here!", {file: "./images/sail.jpg"});
			...
			case 23:
				if (content === "japan" || content === "china" || content === "korea" || content === "netherlands" || content === "spain"){
					...
					channel.send("Hey, is it just me or is the boat suddenly rocking REALLY violently?");
